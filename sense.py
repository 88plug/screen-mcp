...
def classify_activity(before_small, after_small, thresh=12):
    ...
    frac = float((np.abs(a.astype(np.int16) - b.astype(np.int16)).max(axis=2) > thresh).mean())
    act = ("none" if frac < 0.003 else "local" if frac < 0.08
           else "panel" if frac < 0.30 else "major")
...
def change_bbox(before_small, after_small, view, thresh=12):
    ...
    x0, x1 = cols[0], cols[-1]; y0, y1 = rows[0], rows[-1]
    return [int(x0 * sx), int(y0 * sy), int((x1 - x0 + 1) * sx), int((y1 - y0 + 1) * sy)]


def _in_bbox(center, bbox, pad=8):
    x, y, w, h = bbox
    return (x - pad) <= center[0] <= (x + w + pad) and (y - pad) <= center[1] <= (y + h + pad)
...
def scroll_from_pair(before_small, after_small, thresh=12):
    ...
    if hi <= lo:
        lo, hi = 0, h
    frac = float((np.abs(a[lo:hi].astype(np.int16) - b[lo:hi].astype(np.int16)).max(axis=2) > thresh).mean())
...
def _key(el, grid=16):
    ...
    return (el.get("role", ""), _norm(el.get("label", "")), round(cx / grid), round(cy / grid))
...
def diff_elements(prev, cur, move_radius=40):
    ...
    try:
        ...
        def _slim(e):
            return {"role": e.get("role"), "label": e.get("label"),
        ...
        for c in cur:
            ...
            for p in prev:
                if id(p) in used:
                    continue
                if crole == p.get("role", "") and cl and cl == _norm(p.get("label", "")):
                    pcx, pcy = _center(p["bbox"])
                    if abs(pcx - ccx) <= move_radius and abs(pcy - ccy) <= move_radius:
                        hit = p; break
            if hit is not None:
                used.add(id(hit)); moved += 1; continue
            if not cl:  # label-less icon: IoU match
                for p in prev:
                    if id(p) in used or p.get("role", "") != crole or _norm(p.get("label", "")):
                        continue
                    if _iou(c["bbox"], p["bbox"]) > 0.5:
                        hit = p; break
                if hit is not None:
                    used.add(id(hit)); continue
            new.append(_slim(c))
        ...
        return {"new": new, "removed": int(removed), "moved": int(moved), "changed": changed}
    except Exception:
...
def detect_overlay(before_small, after_small, new_elements, view_wh=None):
    ...
    try:
        present = False; kind = "none"; conf = 0.0; region = None
        # Cue A: backdrop dim
        b, a = _aligned(before_small, after_small)
        if b is not None:
            lb = b.astype(np.int16).mean(axis=2); la = a.astype(np.int16).mean(axis=2)
            darker = (la < lb - 18)
            dim_frac = float(darker.mean())
            if dim_frac > 0.25:
                present = True; kind = "modal"; conf = min(1.0, 0.4 + dim_frac)
        ...
        if not present and ne and view_wh:
            vw, vh = view_wh
            xs = [e["center"][0] for e in ne]; ys = [e["center"][1] for e in ne]
            if len(ne) <= 3 and (max(ys) / max(1, vh) > 0.8 or max(xs) / max(1, vw) > 0.85):
                kind = "toast"   # corner notification — informational, not blocking
        return {"present": present, "kind": kind, "confidence": round(conf, 2), "region": region}
    except Exception:
...
def _same_view(v1, v2):
    if not v1 or not v2:
        return False
    return (v1.get("ox") == v2.get("ox") and v1.get("oy") == v2.get("oy")
            and abs((v1.get("scale") or 0) - (v2.get("scale") or 0)) < 1e-6
            and v1.get("dw") == v2.get("dw") and v1.get("dh") == v2.get("dh"))
    ...
    try:
        ...
        if comparable:
            ...
            if meaningful and elements is not None and h.get("elements") is not None:
                ...
                new_elements = new_el
                if new_el:
                    out["change"] = {"new": new_el[:8], "new_count": len(new_el),
            ...
            if meaningful:
                ...
                ov = detect_overlay(prev_small, cur_small, new_elements, (vw, vh) if vw else None)
                if ov["present"]:
                    out["overlay"] = ov
    except Exception:
        pass
