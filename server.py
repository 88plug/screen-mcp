...
TOOLS = [
    ...
    {"name": "screen_list_monitors", "annotations": {"readOnlyHint": True, "destructiveHint": False}, "description": "List monitors (origin, size, scale), desktop bounds, and open windows.",
     "inputSchema": {"type": "object", "properties": {"regeo": {"type": "boolean"}}}},
    {"name": "screen_move_mouse", "annotations": {"readOnlyHint": False, "destructiveHint": False}, "description": "Move mouse to x,y (view-space default). Or dx,dy for a relative nudge.",
     "inputSchema": {"type": "object", "properties": {**_POS, "dx": {"type": "number"}, "dy": {"type": "number"}}}},
    {"name": "screen_click", "annotations": {"readOnlyHint": False, "destructiveHint": True}, "description": "Click at x,y (view-space default; server maps to the real screen). Omit x,y to click in place. button: left|right|middle, double:bool.",
     "inputSchema": {"type": "object", "properties": {**_POS, "button": {"type": "string"}, "double": {"type": "boolean"}}}},
    {"name": "screen_scroll", "annotations": {"readOnlyHint": False, "destructiveHint": False}, "description": "Scroll the wheel. direction up|down|left|right, amount notches. Optional x,y to position first.",
     "inputSchema": {"type": "object", "properties": {**_POS, "direction": {"type": "string"}, "amount": {"type": "number"}}}},
    {"name": "screen_drag", "annotations": {"readOnlyHint": False, "destructiveHint": True}, "description": "Press-drag from (x1,y1) to (x2,y2) (view-space). button: left|middle|right.",
    ...
    {"name": "screen_key", "annotations": {"readOnlyHint": False, "destructiveHint": True}, "description": "Press a key/combo: 'Ctrl+L', 'Enter', 'Alt+Tab', 'F5'. Keys go to the FOCUSED window — pass focus='appname' to focus the target first (a background/static-monitor app won't receive keys otherwise).",
    ...
    {"name": "screen_type", "annotations": {"readOnlyHint": False, "destructiveHint": True}, "description": "Type text (Unicode ok); enter:true presses Enter after. Text goes to the FOCUSED window — pass focus='appname' (or call screen_focus first) so it lands in the right app, not whatever currently has focus.",
    ...
    {"name": "screen_wait", "annotations": {"readOnlyHint": True, "destructiveHint": False}, "description": "Wait until the screen stops changing (settles) or timeout, then optionally screenshot. Use after an action that triggers an async/htmx update — returns as soon as the UI is stable instead of you guessing a fixed delay. Also usable as a 'wait_stable' step inside screen_do/screen_tour. Args: timeout (s, default 5), region/monitor (area to watch), shot.",
    ...
    {"name": "screen_session", "annotations": {"readOnlyHint": False, "destructiveHint": False}, "description": "Session recording/replay: op=start|stop|list|status|replay-path. Records every action + screenshot to a replayable trajectory.",
     "inputSchema": {"type": "object", "properties": {"op": {"type": "string"}, "id": {"type": "string"}}}},
    {"name": "screen_reload", "annotations": {"readOnlyHint": False, "destructiveHint": True}, "description": "Hot-reload this server's own code in place (re-exec, preserving the connection) so edits take effect WITHOUT a /mcp reconnect.",
     "inputSchema": {"type": "object", "properties": {}}},
    {"name": "screen_diag", "annotations": {"readOnlyHint": True, "destructiveHint": False}, "description": "Health dump: session/geo, live cursor position + user-takeover guard state, grounding backends (OCR/OmniParser) and whether they're warmed. Check this first when capture, clicks, or the cursor guard misbehave.",
     "inputSchema": {"type": "object", "properties": {}}},
]
HANDLERS = {
