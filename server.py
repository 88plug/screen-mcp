...
TOOLS = [
    ...
    {"name": "screen_list_monitors", "title": "List Monitors", "annotations": {"readOnlyHint": True, "destructiveHint": False}, "description": "List monitors (origin, size, scale), desktop bounds, and open windows.",
     "inputSchema": {"type": "object", "properties": {"regeo": {"type": "boolean"}}}},
    {"name": "screen_move_mouse", "title": "Move Mouse", "annotations": {"readOnlyHint": False, "destructiveHint": False}, "description": "Move mouse to x,y (view-space default). Or dx,dy for a relative nudge.",
     "inputSchema": {"type": "object", "properties": {**_POS, "dx": {"type": "number"}, "dy": {"type": "number"}}}},
    {"name": "screen_click", "title": "Click", "annotations": {"readOnlyHint": False, "destructiveHint": False}, "description": "Click at x,y (view-space default; server maps to the real screen). Omit x,y to click in place. button: left|right|middle, double:bool.",
     "inputSchema": {"type": "object", "properties": {**_POS, "button": {"type": "string"}, "double": {"type": "boolean"}}}},
    {"name": "screen_scroll", "title": "Scroll", "annotations": {"readOnlyHint": False, "destructiveHint": False}, "description": "Scroll the wheel. direction up|down|left|right, amount notches. Optional x,y to position first.",
     "inputSchema": {"type": "object", "properties": {**_POS, "direction": {"type": "string"}, "amount": {"type": "number"}}}},
    {"name": "screen_drag", "title": "Drag", "annotations": {"readOnlyHint": False, "destructiveHint": False}, "description": "Press-drag from (x1,y1) to (x2,y2) (view-space). button: left|middle|right.",
    ...
    {"name": "screen_session", "title": "Record Session", "annotations": {"readOnlyHint": False, "destructiveHint": False}, "description": "Session recording/replay: op=start|stop|list|status|replay-path. Records every action + screenshot to a replayable trajectory.",
