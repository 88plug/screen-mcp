...
TOOLS = [
    {"name": "screen_screenshot", "title": "Capture Screen", "annotations": {"readOnlyHint": True, "destructiveHint": False}, "description": "Capture the desktop, lossless, auto-sized to the model's native resolution. No args = full multi-monitor overview to LOCATE targets (never assume which monitor). region=[x,y,w,h] or monitor=<i> zooms in crisp. annotate=true overlays numbered Set-of-Marks on detected UI elements + lists their click coords. use_cache=true (with annotate) reuses the world-model's learned elements for a known screen, skipping OCR. Responses carry a SENSE line (what changed: new elements, modal opened, no-op) so you adapt automatically.",
    ...
    {"name": "screen_list_monitors", "title": "List Monitors", "annotations": {"readOnlyHint": True, "destructiveHint": False}, "description": "List monitors (origin, size, scale), desktop bounds, and open windows. Use first when choosing where to screenshot or click. Returns monitor geometry plus window list.",
     "inputSchema": {"type": "object", "properties": {"regeo": {"type": "boolean"}}}},
    {"name": "screen_move_mouse", "title": "Move Mouse", "annotations": {"readOnlyHint": False, "destructiveHint": False}, "description": "Move mouse to x,y (view-space default) or dx,dy relative. Use before click when you need an explicit hover position. Returns after the pointer settles.",
     "inputSchema": {"type": "object", "properties": {**_POS, "dx": {"type": "number"}, "dy": {"type": "number"}}}},
    {"name": "screen_click", "title": "Click", "annotations": {"readOnlyHint": False, "destructiveHint": False}, "description": "Click at x,y (view-space; mapped to the real screen). Omit x,y to click in place. Use for single UI activations. button: left|right|middle; double:bool. Returns action result (and optional post-click shot).",
     "inputSchema": {"type": "object", "properties": {**_POS, "button": {"type": "string"}, "double": {"type": "boolean"}}}},
    {"name": "screen_scroll", "title": "Scroll", "annotations": {"readOnlyHint": False, "destructiveHint": False}, "description": "Scroll the wheel (direction up|down|left|right, amount notches). Use to reveal off-screen content before screenshot. Optional x,y to position first. Returns after scroll settles.",
     "inputSchema": {"type": "object", "properties": {**_POS, "direction": {"type": "string"}, "amount": {"type": "number"}}}},
    {"name": "screen_drag", "title": "Drag", "annotations": {"readOnlyHint": False, "destructiveHint": False}, "description": "Press-drag from (x1,y1) to (x2,y2) in view-space. Use for sliders, reorder, selection. button: left|middle|right. Returns after drag completes.",
    ...
    {"name": "screen_key", "title": "Press Key", "annotations": {"readOnlyHint": False, "destructiveHint": False}, "description": "Press a key/combo: 'Ctrl+L', 'Enter', 'Alt+Tab', 'F5'. Keys go to the FOCUSED window — pass focus='appname' to focus the target first (a background/static-monitor app won't receive keys otherwise).",
    ...
    {"name": "screen_type", "title": "Type Text", "annotations": {"readOnlyHint": False, "destructiveHint": False}, "description": "Type text (Unicode ok); enter:true presses Enter after. Text goes to the FOCUSED window — pass focus='appname' (or call screen_focus first) so it lands in the right app, not whatever currently has focus.",
    ...
    {"name": "screen_focus", "title": "Focus Window", "annotations": {"readOnlyHint": False, "destructiveHint": False}, "description": "Raise + give KEYBOARD FOCUS to a window so injected keys/clicks land in it — keyboard events go to the focused window, which a background or static-monitor app does NOT have (the #1 reason 'I typed but nothing happened'). Match by app ('slack', 'firefox'), title substring, or an explicit window id from screen_list_monitors. Uses the window-info extension's ActivateWindow when loaded; otherwise falls back to the GNOME overview (types the name). Call this before screen_type/screen_key on an app you haven't clicked into.",
     "inputSchema": {"type": "object", "properties": {"app": {"type": "string"}, "title": {"type": "string"}, "id": {"type": ["string", "number"]}}}},
    {"name": "screen_do", "title": "Batch Actions", "annotations": {"readOnlyHint": False, "destructiveHint": True}, "description": "Run an ordered batch of actions in one call to cut round-trips: steps=[{action:'move|click|scroll|drag|key|type|wait|wait_stable', ...that action's args}]. 'wait' sleeps fixed ms; 'wait_stable' blocks until the screen settles (for htmx/async). One optional final screenshot (shot=true, region=[x,y,w,h] to frame it). Stops mid-batch if you take the mouse (force=true to override).",
    ...
    {"name": "screen_read_page", "title": "Read Page", "annotations": {"readOnlyHint": False, "destructiveHint": False}, "description": "Read a whole scrollable view in ONE call: auto-scrolls down until content stops moving, annotating each screen and accumulating every interactable it finds. Returns the full inventory + a final screenshot — instead of you doing N rounds of scroll+screenshot. region defaults to the last view; max_pages caps it; force bypasses the takeover guard. Leaves the current screen clickable by [id].",
    ...
    {"name": "screen_tour", "title": "Tour UI States", "annotations": {"readOnlyHint": False, "destructiveHint": True}, "description": "Visit several UI states in ONE call and get a labeled thumbnail of each — collapses N navigate->screenshot round-trips into 1 (the fast way to survey/navigate). steps=[{label, steps:[{action:'click|scroll|key|type|move|wait', ...}], region?, settle?}]. Steps take the same args as screen_do (element ids ok). shot_max_edge (default 1280) sizes the thumbnails; settle is the per-stop ms before capture. force bypasses the user-takeover guard.",
    ...
    {"name": "screen_wait", "title": "Wait for Settle", "annotations": {"readOnlyHint": True, "destructiveHint": False}, "description": "Wait until the screen stops changing (settles) or timeout, then optionally screenshot. Use after an action that triggers an async/htmx update — returns as soon as the UI is stable instead of you guessing a fixed delay. Also usable as a 'wait_stable' step inside screen_do/screen_tour. Args: timeout (s, default 5), region/monitor (area to watch), shot.",
    ...
    {"name": "screen_session", "title": "Record Session", "annotations": {"readOnlyHint": False, "destructiveHint": False}, "description": "Session recording/replay: op=start|stop|list|status|replay-path. Use to capture a trajectory of actions+screenshots for later replay. Returns status or path for the op.",
     "inputSchema": {"type": "object", "properties": {"op": {"type": "string"}, "id": {"type": "string"}}}},
    {"name": "screen_reload", "title": "Reload Server", "annotations": {"readOnlyHint": False, "destructiveHint": True}, "description": "Hot-reload this MCP server's own code in place (re-exec, preserving the connection). Use after editing server code so tools update WITHOUT /mcp reconnect. Returns after re-exec.",
