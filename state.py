...
MAX_EDGE = int(os.environ.get("MCP_SCREEN_MAX_EDGE", "2576"))  # Opus 4.7 native long edge

bus = Gio.bus_get_sync(Gio.BusType.SESSION, None)
sender = bus.get_unique_name()[1:].replace(".", "_")
_ctr = 0
def tok(p):
    global _ctr; _ctr += 1; return f"{p}{_ctr}"
...
def request(sig, iface, method, lead, options):
    ...
    ht = tok("h")
    options = dict(options); options["handle_token"] = GLib.Variant("s", ht)
    handle = f"{OBJ}/request/{sender}/{ht}"
    out = {}
    loop = GLib.MainLoop()
    def on_resp(c, s, o, i, sg, params):
        code, results = params.unpack()
        out["code"] = code; out["results"] = results; loop.quit()
    sid = bus.signal_subscribe(PORTAL, "org.freedesktop.portal.Request", "Response",
                               handle, None, Gio.DBusSignalFlags.NONE, on_resp)
    args = GLib.Variant(sig, tuple(lead) + (options,))
    bus.call_sync(PORTAL, OBJ, iface, method, args, None, Gio.DBusCallFlags.NONE, -1, None)
    GLib.timeout_add_seconds(120, lambda: (out.setdefault("code", 99), loop.quit()) and False)
    loop.run()
...
SESSION = {"handle": None, "streams": None, "fd": None, "geo": None, "W": 0, "H": 0,
           "view": None, "view_seq": 0, "cursor": None, "cmd_cursor": None, "cmd_node": None, "last_input_t": None, "last_input_node": None, "last_input_hash": None}
...
def ensure_session():
    ...
    if SESSION["handle"]:
        return
    r = request("(a{sv})", RD, "CreateSession", (),
    ...
    request("(oa{sv})", SC, "SelectSources", (sess,),
    ...
    if res.get("restore_token"):
        ...
        open(TOKEN_FILE, "w").write(res["restore_token"]); os.chmod(TOKEN_FILE, 0o600)
    SESSION["handle"] = sess
    SESSION["streams"] = res["streams"]   # list of (node_id:int, props:{position,size,...})
    reply_v, fds = bus.call_with_unix_fd_list_sync(
        PORTAL, OBJ, SC, "OpenPipeWireRemote", GLib.Variant("(oa{sv})", (sess, {})),
...
def open_pw_fd():
    ...
    reply_v, fds = bus.call_with_unix_fd_list_sync(
        PORTAL, OBJ, SC, "OpenPipeWireRemote", GLib.Variant("(oa{sv})", (SESSION["handle"], {})),
        GLib.VariantType("(h)"), Gio.DBusCallFlags.NONE, -1, None, None)
