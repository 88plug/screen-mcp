...
def check_python_deps():
    ...
    for mod in ("numpy", "PIL"):
        try:
            ...
            missing.append(mod)
    if not missing:
        return _entry("python_deps", "ok", f"numpy + Pillow (python {sys.version_info.major}.{sys.version_info.minor})", None)
    return _entry(
        "python_deps", "fail",
...
def check_gstreamer():
    try:
        ...
        if (major, minor) < (1, 28):
            return _entry(
                "gstreamer", "fail", f"{detail} (< 1.28; leaky-type unavailable)",
                "Upgrade to GStreamer >= 1.28",
            )
        return _entry("gstreamer", "ok", detail, None)
    except Exception as e:
        return _entry(
            "gstreamer", "fail", str(e),
...
def check_wayland():
    ...
    if wl or xdg == "wayland":
        return _entry("wayland", "ok", f"session type wayland (WAYLAND_DISPLAY={wl or 'set'})", None)
    return _entry(
        "wayland", "fail",
...
def check_window_info():
    try:
        ...
        if st["loaded"]:
            return _entry("window_info", "ok", "window-info@local loaded on D-Bus", None)
        if st["installed"]:
            return _entry(
                "window_info", "warn",
...
def check_all():
    ...
    for fn in _CHECKS:
        try:
            ...
            items.append(_entry(fn.__name__.removeprefix("check_"), "warn", f"check error: {e}", None))
    summary = {k: sum(1 for i in items if i["status"] == k) for k in ("ok", "warn", "fail")}
